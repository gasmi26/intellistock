...
if __name__ == "__main__":
    app.run(debug=True, port=5000) flask import Flask, jsonify, request
from flask_cors import CORS
import sqlite3
import pandas as pd
import torch
...
def llm_critic(sku, predicted_demand, order_qty,
               total_cost, market_context):
    prompt = f"""
...
@app.route("/")
def home():
    return jsonify({"message": "IntelliStock Backend Running"})
...
def run_pipeline():
    ...
    try:
        ...
        # Extract verdict
        verdict = "PASS" if "VERDICT: PASS" in critic_result else "REVISE"
        ...
        report = {
            ...
            "critic_reasoning": critic_result,
            "ai_report": ai_report,
            "warnings": [critic_result],
            "market_context": market_docs,
        }
